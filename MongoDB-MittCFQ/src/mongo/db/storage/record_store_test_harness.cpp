...
TEST(RecordStoreTestHarness, Simple1) {
...

    {
        ...
        ASSERT_EQUALS(s, rs->dataFor(opCtx.get(), loc1,0).data());
    }

    {
        ...
        RecordData rd;
        ASSERT(!rs->findRecord(opCtx.get(), RecordId(111, 17), &rd,0));
        ASSERT(rd.data() == NULL);

        ASSERT(rs->findRecord(opCtx.get(), loc1, &rd,0));
...
TEST(RecordStoreTestHarness, Simple1InsertDocWroter) {
...

    {
        ...
        ASSERT_EQUALS(string("eliot"), rs->dataFor(opCtx.get(), loc1,0).data());
    }
}
...
TEST(RecordStoreTestHarness, Delete1) {
    ...
    RecordId loc;
    {
        ...
        ASSERT_EQUALS(s, rs->dataFor(opCtx.get(), loc,0).data());
    }
...
TEST(RecordStoreTestHarness, Update1) {
    ...
    {
        ServiceContext::UniqueOperationContext opCtx(harnessHelper->newOperationContext());
        ASSERT_EQUALS(s1, rs->dataFor(opCtx.get(), loc,0).data());
    }
...

    {
        ServiceContext::UniqueOperationContext opCtx(harnessHelper->newOperationContext());
        ASSERT_EQUALS(1, rs->numRecords(opCtx.get()));
        ASSERT_EQUALS(s2, rs->dataFor(opCtx.get(), loc,0).data());
    }
}
