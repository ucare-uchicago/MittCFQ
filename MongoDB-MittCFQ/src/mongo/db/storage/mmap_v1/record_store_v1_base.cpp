...
RecordData RecordStoreV1Base::dataFor(OperationContext* txn, const RecordId& loc, int fromDisk) const {
    return recordFor(DiskLoc::fromRecordId(loc),fromDisk)->toRecordData();
}

bool RecordStoreV1Base::findRecord(OperationContext* txn,
                                   const RecordId& loc,
                                   RecordData* rd, int fromDisk) const {
    ...
    MmapV1RecordHeader* rec = recordFor(DiskLoc::fromRecordId(loc),fromDisk);
    if (!rec) {
        return false;
    }
    *rd = rec->toRecordData();
...
MmapV1RecordHeader* RecordStoreV1Base::recordFor(const DiskLoc& loc,int fromDisk) const {
    return _extentManager->recordForV1(loc,fromDisk);
}

const DeletedRecord* RecordStoreV1Base::deletedRecordFor(const DiskLoc& loc) const {
    invariant(loc.a() != -1);
    return reinterpret_cast<const DeletedRecord*>(recordFor(loc,0));
}

DeletedRecord* RecordStoreV1Base::drec(const DiskLoc& loc) const {
    invariant(loc.a() != -1);
    return reinterpret_cast<DeletedRecord*>(recordFor(loc,0));
}
...
DiskLoc RecordStoreV1Base::getNextRecordInExtent(OperationContext* txn, const DiskLoc& loc) const {
    int nextOffset = recordFor(loc,0)->nextOfs();
...
DiskLoc RecordStoreV1Base::getPrevRecordInExtent(OperationContext* txn, const DiskLoc& loc) const {
    int prevOffset = recordFor(loc,0)->prevOfs();
...
Status RecordStoreV1Base::insertRecordsWithDocWriter(OperationContext* txn,
    ...
    for (size_t i = 0; i < nDocs; i++) {
        ...
        MmapV1RecordHeader* r = recordFor(loc.getValue(),0);
...
Status RecordStoreV1Base::updateRecord(OperationContext* txn,
    ...
    MmapV1RecordHeader* oldRecord = recordFor(DiskLoc::fromRecordId(oldLocation),0);
...
StatusWith<RecordData> RecordStoreV1Base::updateWithDamages(
    ...
    MmapV1RecordHeader* rec = recordFor(DiskLoc::fromRecordId(loc),0);
    char* root = rec->data();
...
void RecordStoreV1Base::deleteRecord(OperationContext* txn, const RecordId& rid) {
    ...
    MmapV1RecordHeader* todelete = recordFor(dl,0);
...
void RecordStoreV1Base::_addRecordToRecListInExtent(OperationContext* txn,
    ...
    if (e->lastRecord.isNull()) {
        ...
        *txn->recoveryUnit()->writing(&e->lastRecord) = loc;
        ...
        MmapV1RecordHeader* oldlast = recordFor(e->lastRecord,0);
    ...
    try {
        ...
        if (level == kValidateRecordStore || level == kValidateFull) {
            ...
            while (auto record = cursor->next()) {
                ...
                MmapV1RecordHeader* r = recordFor(dl,0);
...
boost::optional<Record> RecordStoreV1Base::IntraExtentIterator::next() {
    ...
    advance();
    return {{out, _rs->dataFor(_txn, out,0)}};
}
...
void RecordStoreV1Base::IntraExtentIterator::advance() {
    ...
    const MmapV1RecordHeader* rec = recordFor(_curr,0);
