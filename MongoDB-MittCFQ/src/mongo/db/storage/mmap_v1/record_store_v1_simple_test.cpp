...
TEST(SimpleRecordStoreV1, AllocQuantized) {
    ...
    ASSERT_EQUALS(512, rs.dataFor(&txn, result.getValue(),0).size() + MmapV1RecordHeader::HeaderSize);
}
...
TEST(SimpleRecordStoreV1, AllocNonQuantized) {
    ...
    // The length of the allocated record is quantized.
    ASSERT_EQUALS(300, rs.dataFor(&txn, result.getValue(),0).size() + MmapV1RecordHeader::HeaderSize);
}
...
TEST(SimpleRecordStoreV1, FullSimple1) {
    ...
    RecordData recordData = rs.dataFor(&txn, result.getValue(),0);
