...
StatusWith<DiskLoc> CappedRecordStoreV1::allocRecord(OperationContext* txn,
    ...
    {  // do allocation
        ...
        while (1) {
            ...
            Status status = _cappedCallback->aboutToDeleteCapped(txn, fr, dataFor(txn, fr,0));
            if (!status.isOK())
...
void CappedRecordStoreV1::cappedTruncateAfter(OperationContext* txn,
    ...
    while (1) {
        ...
        Status status = _cappedCallback->aboutToDeleteCapped(txn, currId, dataFor(txn, currId,0));
