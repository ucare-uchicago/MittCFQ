...
public:
    ...
    virtual RecordData dataFor(OperationContext* txn, const RecordId& loc, int fromDisk) const;

    virtual bool findRecord(OperationContext* txn, const RecordId& loc, RecordData* rd, int fromDisk) const;
...
protected:
    virtual MmapV1RecordHeader* recordFor(const DiskLoc& loc,int fromDisk) const;
...
private:
    virtual const MmapV1RecordHeader* recordFor(const DiskLoc& loc, int fromDisk) const {
        return _rs->recordFor(loc,fromDisk);
    }
