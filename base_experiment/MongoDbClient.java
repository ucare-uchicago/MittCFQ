...
public class MongoDbClient extends DB {
	...
	private static int mz_count = 0;

	/** The database name to access. */
	private static MongoDatabase database;
	private static MongoDatabase database2;
	private static MongoDatabase database3;
	...
	private static MongoClient mongoClient3;
	...
	public void init() throws DBException {
		...
		synchronized (INCLUDE) {
			...
			// url =
			// "mongodb://pc702.emulab.net:27018,pc709.emulab.net:27018,pc715.emulab.net:27018/readPreference=nearest";
			String url = "mongodb://pc706.emulab.net:27018/ycsb?w=1";
			String url2 = "mongodb://pc708.emulab.net:27018/ycsb?w=1";
			String url3 = "mongodb://pc712.emulab.net:27018/ycsb?w=1";
			...
			try {
				...
				MongoClientURI uri2 = new MongoClientURI(url2);
				MongoClientURI uri3 = new MongoClientURI(url3);

				String uriDb = uri.getDatabase();
				String uriDb2 = uri2.getDatabase();
				String uriDb3 = uri3.getDatabase();
				...
				mongoClient2 = new MongoClient(uri2);
				database2 = mongoClient2.getDatabase(databaseName).withReadPreference(readPreference)
						.withWriteConcern(writeConcern);
				mongoClient3 = new MongoClient(uri3);
				database3 = mongoClient3.getDatabase(databaseName).withReadPreference(readPreference)
						.withWriteConcern(writeConcern);
	...
	public Status insert(String table, String key, HashMap<String, ByteIterator> values) {
		try {
			...
			if (batchSize == 1) {
				if (useUpsert) {
					// this is effectively an insert, but using an upsert
					// instead due
					// to current inability of the framework to clean up after
					// itself
					// between test runs.
	...
	public Status read(String table, String key, Set<String> fields, HashMap<String, ByteIterator> result) {
		mz_count++;
		MongoDatabase mz_db;
		int mz_index = random.nextInt(3);
		// int mz_index = mz_count%3;
		if (mz_index == 0) {
			mz_db = database;
		} else if (mz_index == 1) {
			mz_db = database2;
		...
		try {
			MongoCollection<Document> collection = mz_db.getCollection(table);
			Document query = new Document("_id", key);
			...
			String entry_key = (String) queryResult.get("_id");
			if (entry_key.equals("user0123456789")) {
				if (mz_index == 0) {
					mz_db = database2;
				} else if (mz_index == 1) {
					mz_db = database3;
				...
				collection = mz_db.getCollection(table);
				...
				entry_key = (String) queryResult.get("_id");
				if (entry_key.equals("user0123456789")) {
					if (mz_index == 0) {
						mz_db = database3;
					} else if (mz_index == 1) {
